...
window.widgetConfig.PERFTEST_INTERACTIONID = window.widgetConfig
  .randomInteractionId(16)
  .toUpperCase();
window.widgetConfig.socket = null;

console.log(
  `RANDOM interactionId => ${window.widgetConfig.PERFTEST_INTERACTIONID}`
);
  ...
  switch (event.type) {
    ...
    case "createVideoRoom": {
      ...
      if (
        ...
        console.log("Videoroom url: " + event.payload.url);
        console.log("InteractionId: " + event.payload.interactionId);
        console.log('Sticky: "' + event.payload.sticky);
...
window.widgetConfig.startVideo = function (
  roomId,
  username,
  credential,
  url,
  ...
  var dataPayload = JSON.stringify({
    videoActionType: "VIDEO_ROOM_CREATED",
    roomId: roomId,
  ...
  //  "url": "turn:gateway-widgetlab.o2.cz:8445?transport=tcp"
  //},
  //  "isVisible":false
  //}
};
...
window.widgetConfig.startSocketIO = function () {
  if (window.widgetConfig.socket != null) return;

  window.widgetConfig.socket = io(
    `${window.widgetConfig.PERFTEST_SOCKET_URL}`,
    {
...
window.chrome.webview = window.chrome.webview || {
  ...
  addEventListener: function (eventName, listener) {
    ...
    else window.chrome.webview.callbacks[eventName] = [listener];
  },
  executeEventListener: function (eventName, payload) {
    for (
      var i = 0, len = window.chrome.webview.callbacks[eventName].length;
      i < len;
      i++
    )
